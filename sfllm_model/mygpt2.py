...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config: GPT2Config):
        ...
        self.rope = RotaryEmbedding(
            dim=self.embed_dim // config.num_attention_heads)
    ...
    def forward(
        ...
        def rope_fn(q: torch.Tensor, k: torch.Tensor):
            assert q.size(-2) == k.size(-2)
            seqlen = q.size(-2)
            pos_emb = self.rope.get_emb(
                position_ids) if position_ids is not None else self.rope.forward(seqlen)
