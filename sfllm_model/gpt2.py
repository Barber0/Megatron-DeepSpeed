...
class GPT2Attention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None, rope_fn=None):
        super().__init__()
    ...
    def forward(
        ...
        # =====================
        seq_dim_idx = -2
        qlen = query.size(seq_dim_idx)
        klen = key.size(seq_dim_idx)
        q_offset = klen - qlen
        query = self.rope_fn(query, offset=q_offset)
        key = self.rope_fn(key)
        # =====================
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None, rope_fn=None):
        super().__init__()
        hidden_size = config.hidden_size
        inner_dim = config.n_inner if config.n_inner is not None else 4 * hidden_size

        self.ln_1 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.attn = GPT2Attention(config, layer_idx=layer_idx, rope_fn=rope_fn)
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([GPT2Block(config, layer_idx=i, rope_fn=self.rope.rotate_queries_or_keys)
